...
export const ProfilePage = {
  render(state) {
    ...
    const identitySection = isGuest
      ...
      : `
        <div class="flex items-center gap-4">
          <div class="w-14 h-14 rounded-2xl bg-slate-900 text-white flex items-center justify-center text-xl font-bold flex-shrink-0">
            ${initials}
          </div>
          <div class="flex flex-col gap-0.5">
            <h2 class="text-lg font-bold text-text-primary leading-tight">@${username}</h2>
    ...
    const storageCard = isGuest
      ? `
        <div class="flex flex-col gap-1.5 px-2 animate-fade-up delay-2">
          ...
          <div class="flex items-center gap-3 p-3.5 bg-surface-card border border-divider rounded-2xl shadow-sm">
            <div class="w-9 h-9 rounded-xl bg-slate-100 text-slate-700 dark:bg-slate-800 dark:text-slate-200 flex items-center justify-center flex-shrink-0">
      ...
      : `
        <div class="flex flex-col gap-1.5 px-2 animate-fade-up delay-2">
          <span class="text-[9px] font-black text-text-secondary uppercase tracking-[0.2em]">Storage</span>
          <div class="flex items-center gap-3 p-3.5 bg-surface-card border border-divider rounded-2xl shadow-sm">
            <div class="w-9 h-9 rounded-xl bg-slate-900 text-white flex items-center justify-center flex-shrink-0">
      ...
      ? `
        <div class="flex flex-col gap-1.5 px-2 animate-fade-up delay-3">
          ...
          <button
            ...
            class="w-full text-left p-4 border border-divider bg-surface-card rounded-2xl shadow-sm hover:shadow-md hover:border-slate-800 dark:hover:border-slate-400 transition-all group flex items-center gap-3"
          >
            <div class="w-9 h-9 rounded-xl bg-slate-900 text-white flex items-center justify-center flex-shrink-0">
